...
def batchsize_plot():
    ...
    for i in range(4):
        ...
        train_loss_list, Epoch = train(model)
    ...
    plt.title("change batch_sizes in train", fontsize=16)
...
def hiddensize_plot():
    ...
    for i in range(3):
        ...
        train_loss_list, Epoch ,test_loss= train(model)
        train_loss_dict['{}'.format(i)] = train_loss_list
        Epoch_dict['{}'.format(i)] = Epoch
    ...
    y_ticks = np.arange(0, 1, 0.05)
    plt.yticks(y_ticks)  # 设置刻度
    plt.title("change hidden_sizes in train", fontsize=16)
    plt.xlabel("Epoch", fontsize=14)
    plt.ylabel("loss", fontsize=14)
    plt.legend()
    plt.show()
...
def embeddingsize_plot():
    ...
    y_ticks = np.arange(0, 1, 0.05)
    plt.yticks(y_ticks)  # 设置刻度
    plt.title("change embedding_sizes in train", fontsize=16)
    plt.xlabel("Epoch", fontsize=14)
    plt.ylabel("loss", fontsize=14)
    plt.legend()
    plt.show()
...
def dropout_plot():
    ...
    y_ticks = np.arange(0, 1, 0.05)
    plt.yticks(y_ticks)  # 设置刻度
    plt.title("change dropout in train", fontsize=16)
    plt.xlabel("Epoch", fontsize=14)
    plt.ylabel("loss", fontsize=14)
    plt.legend()
    plt.show()
